...
func main() {
	...
	if dir := exportDir(os.Args[1:]); dir != "" {
		...
		// Stamp each page with the language it is written in. GoFastr takes
		// the document language from one host-wide value, so the export
		// labels every page the same, and Pagefind then indexes the whole
		// site as one language.
		if err := built.router.WriteExportLocales(dir); err != nil {
			panic(err)
		}
