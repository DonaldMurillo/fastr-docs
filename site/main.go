...
// componentStylesheet matches a per-component stylesheet link the static
// export writes, capturing the component name.
var componentStylesheet = regexp.MustCompile(`<link rel="stylesheet" href="([^"]*/__gofastr/comp/)([a-z0-9-]+)(\.css[^"]*)">`)

// markComponentStylesheets gives each exported component stylesheet link the
// data-fui-style marker GoFastr's runtime dedupes on.
//
// The runtime loads a component's CSS when it finds the component in the
// page and no link carries its marker. The static export writes the links
// without it (GoFastr through v0.84.0), so on every page the runtime appended
// a second copy of each component stylesheet after app.css, and the site's
// own overrides lost the cascade: the docs sidebar stretched, the doc layout
// collapsed. Marking the links here is what the live host's bundle link
// already gets.
